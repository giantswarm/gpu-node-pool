...
The bootstrap also lets a workload **lock its memory**: the drop-in `20-memlock.conf` on `containerd.service`,
...
`CAP_IPC_LOCK`, root included, so under the default a serving runtime that `mlock()`s its weights (a
unified-memory stack locks every weight allocation so the resident weights survive file readahead) fails its
first lock and dies at engine start with an out-of-memory at a few MiB while the node has a hundred GiB free;
granting the capability to the container is refused under the baseline and restricted Pod Security Standards.
With the limit unlimited the lock is bounded by the container's memory limit, which the cgroup charges for the
locked pages (`systemctl show containerd -p LimitMEMLOCK` on the node reads `infinity`). A GPU node not
created by the pool needs the same drop-in.
...
A pool node has a root volume (`pool.volumes.root`, gp3, 15 GiB), `/var/log` on a gp3 volume of
`pool.volumes.log` (30 GiB) and `/var/lib` — the container image unpacks, the kubelet's directories,
the pods' emptyDirs (a model's weights, where no cache claim is mounted) and the containers'
writable layers — from `pool.volumes.libSource`:

- **`instance-store`** (default): the node's own NVMe instance store. Every size of the curated
  families has one, local to the host and included in the instance price: 250 GB on `xlarge`, 450
  on `2xlarge`, 600 on `4xlarge` and 900 on `8xlarge` of g6, g6e and g5; 125, 225, 225 and 900 GB
  on g4dn. The store's device name is not stable (EBS volumes are NVMe devices too), so the
  bootstrap's `format-instance-store.service` — a oneshot ahead of `var-lib.mount`, which
  `local-fs.target` requires — finds the first block device of model `Amazon EC2 NVMe Instance
  Storage`, formats it as xfs labelled `lib` unless it carries that label already, and
  `var-lib.mount` mounts `/dev/disk/by-label/lib` as on any node. No lib volume is provisioned: the
  `/dev/xvdd` mapping and the Ignition `lib` filesystem are not rendered. The `ec2NodeClass` carries
  `instanceStorePolicy: RAID0`, Karpenter's word for "the instance store is the node's ephemeral
  storage": on a custom image the bootstrap prepares the disk and the policy makes Karpenter count
  its size in scheduling (without it, the root volume's). The data is gone with the node — as the
  lib volume was, deleted on termination. A size without an instance store fails the unit, which
  names the disks present in its journal, and stops the boot there instead of joining the cluster
  with `/var/lib` on the root volume; Karpenter replaces the node after its registration timeout.
  The `8xlarge` of g6 and g6e have two devices of 450 GB; the first is used, the second stays idle.
- **`ebs`**: a provisioned gp3 volume on `/dev/xvdd` of `pool.volumes.lib` (200 GiB) whose
  performance bounds the image pulls on a fresh node. `pool.volumes.libThroughput` (MiB/s, default
  500) and `pool.volumes.libIops` (default 4000) set the volume's `ebs.throughput` and `ebs.iops`.
  gp3 allows 125 to 1000 MiB/s and 3000 to 16000 IOPS with at most 0.25 MiB/s per provisioned
  IOPS; the chart refuses a pair beyond that ratio, which EC2 would refuse at launch.

The root and log volumes stay at gp3's baseline (125 MiB/s, 3000 IOPS). Changing the source
changes the bootstrap and rolls the pool's nodes.
...
an empty `pool.zones` renders no zone requirement), renders `pool.volumes.libSource: ebs` into its
own golden (the default goldens hold the instance store: the format unit and its script, no lib
filesystem entry, no lib block device mapping, `instanceStorePolicy: RAID0`), renders the `KubeadmConfig` of
