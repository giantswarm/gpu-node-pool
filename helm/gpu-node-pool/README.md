...
also the [prewarm](#prewarming-the-first-node) PriorityClass and Job:
...
| `pool.prewarm` | [launch the first node at install](#prewarming-the-first-node) with a preemptible placeholder Job; the installation's own pool only | the person, through cluster-manager |
...
`pool.prewarm.holdMinutes`, under a **PriorityClass** `<cluster>-<pool>-prewarm` of value -1000
with `preemptionPolicy: Never`. Karpenter launches the node for the placeholder; the first
workload (priority 0) preempts the placeholder and takes its GPU.
...
renders the prewarm pair into its own golden, checks the chart's refusals (prewarm off the
management cluster, a lib volume beyond gp3's throughput-per-IOPS ratio), and diffs the
